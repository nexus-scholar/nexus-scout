...
class ValidateProtocolJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if ($validationPassed) {
            ...
            \App\Services\WorkflowOrchestrator::dispatchNext($this->thread, 'validate_protocol', ['passed' => true]);
        } else {
            broadcast(new AgentNodeCompleted($this->thread->id, 'validate_protocol', ['status' => 'failed', 'reason' => 'Query missed golden seeds']));

            if ($loopCount >= 3) {
                $this->thread->update(['status' => ThreadStatus::Failed]);
            ...
            \App\Services\WorkflowOrchestrator::dispatchNext($this->thread, 'validate_protocol', ['passed' => false]);
        }
    }
}
